...
with col2:
    url_input = st.text_input("Enter Company Website URL Target:", value=["https://sigmoidal.io","https://www.growthrocks.com"])
    ...
    if st.session_state.profile_database:
        ...
        for item in st.session_state.profile_database:
            with st.expander(f"Profile: {item.get('company_name')} ({item.get('website_name')})"):
                ...
                st.markdown(f"**✉️ Contact Emails Discovered:** {', '.join(item.get('mail', []))}")
